...
def crawl_product_rank_within_keywords(keywords, product_id):
    search_results = {}
    with ProcessPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(crawl_products, keyword, 1): keyword for keyword in keywords}
    ...
    for keyword in keywords:
        try:
            idx = search_results[keyword].index(product_id)
        except ValueError:
            idx = -1
        search_ranks[keyword] = idx
